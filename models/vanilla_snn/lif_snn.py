...
class LIFLayer(nn.Module):
    ...
    def forward(
        ...
        batch and time together during BPTT: every "timestep" the network
        saw during the recurrence was actually a different flight, not a
        moment within the same flight. This produces exactly the pattern
        seen on the c182 cloud run -- partial, unstable learning (val_acc
        creeping up instead of flat, but never clean) with persistent
        non-finite-batch skips that never fully go to zero, because the
        scrambled recurrent state occasionally produces extreme values
        when very different flights get treated as adjacent timesteps.
        Fixed by removing the auto-transpose entirely (same fix already
        verified clean on c172x -- see CHANGES_trim_fix_pass.md there).
